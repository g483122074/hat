...
from net.THSV_transform import RGB_THSV
...
class CIDNet(nn.Module):
    def __init__(self, 
        ...
        self.trans = RGB_THSV().cuda()
        self.ema=EMA(3)

    def forward(self, x):
        dtypes = x.dtype
        x=self.ema(x)
        hvi = self.trans.RGB_to_THSV(x)
        ...
        output_rgb = self.trans.THSV_to_RGB(output_hvi)

        return output_rgb
    
    def RGB_to_THSV(self,x):
        hvi = self.trans.RGB_to_THSV(x)
        return hvi
